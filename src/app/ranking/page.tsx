...
import {
  Trophy, Crown, Medal, X, LogIn, Vote
} from 'lucide-react';
...
import { getCurrentWeekRanking, getLastWeekRanking, getMyCurrentWeekRanking } from '@/libs/api/ranking/rankingApi';
import { VotePetRanking, MyVoteRankingResponse } from '@/libs/interface/api/ranking/rankingResponseInterface';
import { useUserStore } from '@/libs/store/user/userStore';

export default function RankingPage() {
  const [currentWeekRankings, setCurrentWeekRankings] = useState<VotePetRanking[]>([]);
  const [lastWeekRankings, setLastWeekRankings] = useState<VotePetRanking[]>([]);
  const [myRanking, setMyRanking] = useState<MyVoteRankingResponse | null>(null);
  ...
  const loadMyRanking = async () => {
    ...
    try {
      const response = await getMyCurrentWeekRanking();
          ...
          <CuteCard className="text-center relative overflow-visible">
            ...
            <div className={`${isFirst ? 'pt-6 pb-4' : 'pt-4 pb-3'} px-2`}>
              ...
              <div className={`flex items-center justify-center space-x-2 ${isFirst ? 'mb-3' : ''}`}>
                <Trophy className={`${isFirst ? 'w-4 h-4 md:w-5 md:h-5' : 'w-3 h-3 md:w-4 md:h-4'} text-yellow-500`} />
        ...
        >
          <CuteCard>
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">로그인하고 내 투표 랭킹을 확인하세요!</p>
              <Link href="/login">
        ...
        >
          <CuteCard>
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 mb-4">투표하고 랭킹에 도전해보세요!</p>
              <Link href="/vote">
                <CuteButton variant="primary" size="md">
                  투표하러 가기
                </CuteButton>
          ...
          <div className="flex items-center p-4 md:p-6">
            ...
            {myRanking.profileImageUrl ? (
              <div
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setExpandedImage(myRanking.profileImageUrl);
                }}
              >
                <Image
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-lg md:text-xl text-gray-900 mb-1">
                {myRanking.nickname || myRanking.name}
              </h3>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-gray-900">
                    {myRanking.weeklyVoteCount.toLocaleString()}표
                  </span>
                </div>
              </div>
            </div>
  ...
  const renderPodium = (rankings: VotePetRanking[], title: string) => {
    ...
    const otherRankings = rankings.filter(item => item.rank > 3);
